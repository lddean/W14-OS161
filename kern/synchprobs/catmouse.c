 ...
 */
#if OPT_A1
struct lock *round_lock;
struct lock *bowl_lock;
struct cv *cateating;
struct cv *mouseEating;
volatile int *bowl_array;
volatile int cateatingnum = 0;
volatile int mouseeatingnum = 0;
#endif
               ...
               unsigned long catnumber)
{
    ...
    for(i=0;i<NumLoops;i++) {
        ...
        lock_acquire(round_lock);
        while (mouseeatingnum != 0 || cateatingnum >= NumBowls){
            cv_wait(cateating, round_lock);
        }
        int num_bowls;
        for (num_bowls = 0; num_bowls < NumBowls; num_bowls ++){
            //kprintf("num_bowls  %d  situatione %d \n", num_bowls ,bowl_array[num_bowls]);
            if (bowl_array[num_bowls] == 0){
                break;
            }
        }
        //lock_acquire(bowl_lock);
        
        cateatingnum = cateatingnum + 1;
        bowl_array[num_bowls] = 1;
        
        lock_release(round_lock);
        ...
        cateatingnum = cateatingnum - 1;
        bowl_array[num_bowls] = 0;
        
        //lock_release(bowl_lock);
        
        cv_broadcast(mouseEating, round_lock);
        
        kprintf("asdfasdfasdfadasfasd");
                 ...
                 unsigned long mousenumber)
{
    ...
    for(i=0;i<NumLoops;i++) {
         ...
         * the rules when it eats */
        lock_acquire(round_lock);
        while(cateatingnum != 0 || mouseeatingnum >= NumBowls){
            cv_wait(mouseEating, round_lock);
        }
        int num_bowls;
        for (num_bowls = 0; num_bowls < NumBowls; num_bowls ++){
            if (bowl_array[num_bowls] == 0){
        ...
        mouseeatingnum = mouseeatingnum + 1;
        bowl_array[num_bowls] = 1;
        ...
        cv_broadcast(cateating, round_lock);
        lock_release(round_lock);
        
        /* legal bowl numbers range from 1 to NumBowls */
        //bowl = ((unsigned int)random() % NumBowls) + 1;
        //mouse_eat(bowl, MouseEatTime);
        //cat_simulation(NULL, 0);
         ...
         char ** args)
{
     ...
     * Start NumCats cat_simulation() threads.
     struct lock *catbowl;
     struct lock *mousebowl;
     struct cv *cateating;
     struct cv *mouseEating;
     int *bowl;
     volatile int cateatingnum;
     volatile int mouseeatingnum;
     */
    
#if OPT_A1
    
    round_lock = lock_create("round_lock");
    cateating = cv_create("cateating");
    mouseEating = cv_create("mouseEating");
    bowl_lock = lock_create("bowl_lock");
    bowl_array = kmalloc(sizeof(int) * NumBowls);
